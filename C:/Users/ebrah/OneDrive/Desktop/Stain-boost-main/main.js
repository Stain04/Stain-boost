...
// Local file:// navigation helper. Production uses clean URLs like /pricing,
// but double-clicking index.html makes those point to C:\pricing on Windows.
(function () {
  ...
  function localPathFor(pathname) {
    ...
    if (path === '/pricing') return 'pricing.html';
    if (path === '/reviews') return 'reviews.html';
    if (path === '/faq') return 'faq.html';
    if (path === '/dashboard') return 'dashboard.html';
    if (path === '/login') return 'login.html';
    if (path === '/admin') return 'admin.html';
    if (path === '/blog') return 'blog/index.html';

    var blog = path.match(/^\/blog\/([a-z0-9-]+)$/i);
    if (blog) return 'blog/' + blog[1] + '.html';

    var track = path.match(/^\/track\/(SB-[A-Z0-9]{4}-[A-Z0-9]{4})$/i);
    if (track) return 'track.html?token=' + encodeURIComponent(track[1].toUpperCase());

    var review = path.match(/^\/review\/(SB-[A-Z0-9]{4}-[A-Z0-9]{4})$/i);
    if (review) return 'review.html?token=' + encodeURIComponent(review[1].toUpperCase());
  ...
  document.addEventListener('click', function (e) {
    ...
    var url = new URL(href, 'https://local.stainboost');
    var mapped = localPathFor(url.pathname);
    if (!mapped) return;
    e.preventDefault();
    var prefix = rootPrefix();
    if (prefix && mapped.indexOf('blog/') === 0) mapped = mapped.slice(5);
    window.location.href = prefix + mapped + (url.hash || '');
  });
})();
